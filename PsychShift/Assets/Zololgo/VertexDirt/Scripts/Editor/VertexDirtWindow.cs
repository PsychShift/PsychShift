/* 
	...
	Copyright 2014-2020, Zoltan Farago, All rights reserved.
*/
using UnityEditor;
using UnityEngine;
class VertexDirtWindow : EditorWindow {
	enum VDFilterRendererMode { AllActiveRenderer, RenderersInSceneViewLayers, SelectedRenderers, LightmappedStaticRenderers }
	...
	[MenuItem ("Tools/Zololgo/VertexDirt/Bake Ambient Occlusion", false, 10)]
	static void ShowWindow () {
		if (!window){
			...
			#else 
				window.title = "VertexDirt Bake AO";
			#endif
			window.ShowUtility();
    ...
    void OnGUI() {
		...
		VertexDirt.settings.samplingDistance = EditorGUILayout.Slider(VertexDirt.settings.samplingDistance,0.01f, 100.0f);
		//
		GUILayout.Space(5);
		...
		GUILayout.Space(20);
		GUILayout.Label ("AO Occluder layers", GUILayout.Width(150));
		GUILayout.BeginHorizontal();
			VertexDirt.settings.useSceneViewLayers = GUILayout.Toggle(VertexDirt.settings.useSceneViewLayers, "",GUILayout.Width(20));
			GUILayout.Label ("Active Scene View Layers only", GUILayout.Width(200));
		GUILayout.EndHorizontal();	
		if (VertexDirt.settings.useSceneViewLayers) {
			VertexDirt.settings.bakeLayerMask = Tools.visibleLayers;
		} else {
			VertexDirt.settings.bakeLayerMask = ~0;
