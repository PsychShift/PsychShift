...
public class VertexDirtSettingsStruct
{
	...
	// useSceneViewLayers
	public bool useSceneViewLayers = false;
}
...
public static class VertexDirt
{
    ...
    public static void Dirt(Transform[] sels) 
	{
		...
		int pnum = 0;
		#if UNITY_EDITOR
		float tempTime = (float)EditorApplication.timeSinceStartup;
		...
		//EditorUtility.DisplayDialog("VertexDirt baking", "Baking of "+sels.Length+" objects ("+pnum+" vertices)\nis finished in "+Mathf.RoundToInt((float)EditorApplication.timeSinceStartup - tempTime)+" seconds.", "OK");
		...
		if (go != null) {
			...
			if (ch != null) {
				...
				if (mf != null) {
					...
					if (ch.originalMesh != null && ch.originalMesh.colors != null && ch.originalMesh.colors.Length > 0) {
